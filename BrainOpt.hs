...
optimize code@(Increment 1:Increment 1:_) = Increment (length $ takeWhile (==Increment 1) code):(optimize $ dropWhile (==Increment 1) code)
-- For multiple + in a row
optimize code@(Decrement 1:Decrement 1:_) = Decrement (length $ takeWhile (==Decrement 1) code):(optimize $ dropWhile (==Decrement 1) code)
-- Same for -
optimize code@(GoRight 1:GoRight 1:_)     = GoRight (length $ takeWhile (==GoRight 1) code):(optimize $ dropWhile (==GoRight 1) code)
-- Same for >
optimize code@(GoLeft 1:GoLeft 1:_)       = GoLeft (length $ takeWhile (==GoLeft 1) code):(optimize $ dropWhile (==GoLeft 1) code)
-- Same for <
optimize [End]                            = [End]
optimize (x:xs)                           = x: optimize xs -- Skip if no optimizations can be applied
